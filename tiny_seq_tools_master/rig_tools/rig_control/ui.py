...
class SEQUENCER_PT_rig_control(bpy.types.Panel):
    ...
    def draw_limb_column(self, col, side):
        ...
        # Draw Arm
        arm = rig_prefs.arm
        hand_str = rig_prefs.hand.replace(".","")
        nudge = rig_prefs.nudge
        if bone.id_data.tiny_rig.is_ik:
            self.draw_nudge_row(col, side, arm, nudge)
            self.draw_ik_row(col, side, arm,)
        col.separator()
        hand_row = col.row(align=True)
        if bone.id_data.tiny_rig.is_ik:
            hand_row.prop(
                bpy.context.window_manager.tiny_rig_ui,
                f"{side}{hand_str}{nudge}",
                icon="SORT_DESC",
                text="",
            )
        hand_row.prop(bone, f'["{side}{hand_str}"]', text=f"{side.replace('_','')} {hand_str}")
    ...
    def draw_pose_row(self, layout, name):
        ...
        pose_row.operator("rigcontrol.set_turnaround", icon="BACK", text="").values =  (body_offset*-1, -1,)
        pose_row.prop(bone, f'["{name}"]')
        pose_row.operator("rigcontrol.set_turnaround", icon="FORWARD", text="").values = ( body_offset, 1)
